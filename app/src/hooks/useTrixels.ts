...
export function useWorldTrixels(
  ...
  const useResolutionTrixels = (resolution: number) => {
    return useQuery<TrixelData[], Error>({
      queryKey: ['resolutionTrixels', worldPubkey.toString(), resolution],
      queryFn: async () => {
        console.log(`[useTrixels] Querying ALL resolutionTrixels for Res: ${resolution}`);
        
        const trixelIds = getAllTrixelsAtResolution(resolution);
        console.log(`[useTrixels] Found ${trixelIds.length} theoretical trixel IDs for Res ${resolution}.`);
        ...
        console.log(`[useTrixels] Successfully generated ${mappedData.length} trixels for Res ${resolution}.`);
        return mappedData;
      },
      enabled: !!program,
      staleTime: 5 * 60 * 1000,
  ...
  const prefetchResolutionTrixels = useCallback(async (resolution: number) => {
    ...
    console.log(`[useTrixels] Attempting to prefetch ALL trixels for Res ${resolution}`);
    ...
    try {
      const calcTrixelQueryKey = ['resolutionTrixels', worldPubkey.toString(), resolution];
      const existingData = queryClient.getQueryData(calcTrixelQueryKey);
      
      if (!existingData) {
        console.log(`[useTrixels] Prefetching data for Res ${resolution} as it's not in cache.`);
        await queryClient.prefetchQuery({
          queryKey: calcTrixelQueryKey,
          queryFn: async () => {
            const trixelIds = getAllTrixelsAtResolution(resolution);
            const onChainTrixelsData = onChainTrixelsQuery.data || [];
            const onChainTrixelsMap = new Map<number, TrixelData>(
        ...
        console.log(`[useTrixels] Data for Res ${resolution} already in cache, prefetch skipped.`);
      }
    } catch (error) {
      console.error(`[useTrixels] Error during prefetch for Res ${resolution}:`, error);
    }
  }, [queryClient, program, worldPubkey.toString(), onChainTrixelsQuery.data, onChainTrixelsQuery.isLoading, onChainTrixelsQuery.isError]);
