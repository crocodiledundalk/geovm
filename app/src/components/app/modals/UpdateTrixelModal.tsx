...
export function UpdateTrixelModal({ 
  ...
  const { refreshTrixels } = useWorldTrixels(worldPubkey, { canonicalResolution: 8 });
        ...
        success: async (signature: string) => {
          try {
            if (onUpdateSuccess) {
              ...
              const numericId = trixelId.toNumber();
              const ancestors = getTrixelAncestors(numericId);
              await refreshTrixels([numericId, ...ancestors]);
            }
          } catch (refreshError) {
